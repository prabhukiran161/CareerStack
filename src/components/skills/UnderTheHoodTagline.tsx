...
import { SKILLS_CONFIG, getResponsive } from "../../config/skills.config";
...
export const UnderTheHoodTagline = () => {
  ...
  const bottomOffset = getResponsive(
    SKILLS_CONFIG.tagline.bottomOffset,
    isMobile,
  );

  return (
    <div
      className={`absolute ${bottomOffset} w-full text-center z-50 pointer-events-none flex items-center justify-center md:space-x-6`}
    >
      {/* Decorative Line Left */}
      <motion.div
        className="hidden md:block w-16 h-[1px] bg-gradient-to-r from-transparent to-white/30 relative origin-right"
      ...
      <motion.h2
        className="text-[22px] md:text-3xl font-light tracking-[0.3em] text-gray-200"
      ...
      <motion.div
        className="hidden md:block w-16 h-[1px] bg-gradient-to-l from-transparent to-brand-red/40 relative origin-left"
        initial={{ scaleX: 0.7, x: 40, opacity: 0 }}
        animate={
          introState !== "idle" || master.debug.disableTagline
            ? { scaleX: 1, x: 0, opacity: 1 }
